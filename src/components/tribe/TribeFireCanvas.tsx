...
const SHAKE_DECAY = 0.94; // медленнее затухание = пружинистость
const SHAKE_TREMOR = 0.45; // ×3 амплитуда колебаний
...
const FIRE_ATTRACT_ALL = 0.038; // gentle pull when all whites converted
const FIRE_ATTRACT_INSIDE = 0.095; // strong pull when all orange inside circle
const SPARK_COUNT = 12;
const SPARK_SPEED = 3.5;
const SPARK_LIFE = 0.4;
const GATHER_RANGE = 95;
const GATHER_EXPAND_DURATION = 2;
const GATHER_EXPAND_AMOUNT = 80; // ограничено, чтобы круг не выходил за границы canvas (max ~280 от центра при 640 height)

// Seeded random for deterministic white node positions
...
export default function TribeFireCanvas() {
  ...
  useEffect(() => {
    orangeNodes.forEach((n) => {
      if (!physicsRef.current.has(n.id)) {
        ...
        // Тангенциальная скорость против часовой стрелки: (-sin θ, cos θ)
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (phase === "horse") {
        // No black fill — transparent background so gradient shows through
        raf = requestAnimationFrame(draw);
        return;
      }
      ...
      // Physics: update positions first (only hovered ball stops)
      let shakeAddX = 0;
      let shakeAddY = 0;
      physicsRef.current.forEach((p) => {
        if (hoveredId === p.id) {
      ...
      // Orange vs orange: store prev when separate; when clustered — light damping
      // Attraction to fire: same for all (single & paired)
      ...
      // Collision: orange vs orange — resolve overlap, then restore speed for separated
      ...
      // Restore velocity only when ball JUST left the drag (was paired, now single)
      // Singles: never restore — let attraction accumulate
      ...
      // Orange: center repel, boundary, drift, speed limit (skip for hovered — stays still)
      // When all orange: touch fire = slow down (like collision), not hard bounce
      physicsRef.current.forEach((p) => {
        ...
        if (toCenter + NODE_R > bounceR) {
          ...
          const dot = p.vx * dir.x + p.vy * dir.y;
          const impactStrength = Math.max(0, dot); // скорость влета в границу
      ...
      // White: boundary, drift, speed limit (skip drift for hovered — gradual stop)
      whitePhysicsRef.current.forEach((p) => {
        if (hoveredId === p.id) return;
        const dist = Math.hypot(p.x - CX, p.y - CY);
      ...
      // Collision: orange vs white (hovered balls stay still — zero orange after, white damped above)
      orangeArr.forEach((a) => {
      ...
      // Circle boundary (with shake + tremor; dashed + rotating when orange selected)
      ctx.save();
      ctx.translate(
      ...
      // Fire icon — grows and burns when hovered
      const firePath =
      ...
      // Line from selected to cursor (or to hovered white ball center when targeting)
      ...
      // Pulling animation (ease-out-back for rubberiness)
      if (pullingNode) {
